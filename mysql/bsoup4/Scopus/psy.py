...
while page is not None:
    url = "https://www.scopus.com/results/results.uri?sort=plf-f&src=s&sid=65f6ea6908b52c9e822ed7d0700fb0ec&sot=aff&sdt=aff&sl=34&s=AF-ID%2860069385%29+AND+SUBJAREA%28PSYC%29&cl=t&offset="+str(offset)+"&origin=resultslist&ss=plf-f&ws=r-f&ps=r-f&cs=r-f&cc=10&txGid=739599655dec70b0b9e118e9e16b01fa"
    ...
    for item in result.findAll('tr', {"class": "searchArea"}):
        ...
        judul = h2.text
        se = db.cursor()
        se.execute("SELECT Title FROM psychology" )
        rs = se.fetchall()
        ...
        if judul not in out :
            ...
            if judul not in out :
                ...
                print("jumlah data psychology =" + str(len(data3)))
    ...
    offset = i
     ...
     sql = "INSERT INTO psychology(Title, Author, Detail, Date_Released, Source, Link_Source, Cited_By) VALUES (%s, %s, %s, %s, %s, %s, %s)" 
     print(len(datae))   
            # try:
     i = 0
     for values in datae:
