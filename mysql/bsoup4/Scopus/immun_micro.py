...
while page is not None:
    url = "https://www.scopus.com/results/results.uri?sort=plf-f&src=s&sid=79c0a31a164d5730096ed758bf198dad&sot=aff&sdt=aff&sl=34&s=AF-ID%2860069385%29+AND+SUBJAREA%28IMMU%29&cl=t&offset="+str(offset)+"&origin=resultslist&ss=plf-f&ws=r-f&ps=r-f&cs=r-f&cc=10&txGid=82c8721b488cebdf10322b72e7fd7f7b"
    ...
    for item in result.findAll('tr', {"class": "searchArea"}):
        ...
        judul = h2.text
        se = db.cursor()
        se.execute("SELECT Title FROM immunology_and_microbiology" )
        rs = se.fetchall()
        ...
        if judul not in out :
            ...
            if ze == 3 :
                ...
                print("jumlah data immunology_and_microbiology =" + str(len(data3)))
                if i == 1:
    ...
    sql = "INSERT INTO immunology_and_microbiology(Title, Author, Detail, Date_Released, Source, Link_Source, Cited_By) VALUES (%s, %s, %s, %s, %s, %s, %s)" 
