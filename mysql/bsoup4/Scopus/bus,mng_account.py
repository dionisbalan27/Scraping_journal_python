...
while page is not None:
    url = "https://www.scopus.com/results/results.uri?sort=plf-f&src=s&sid=ef30b97ae97c738f4a5e337403a03790&sot=aff&sdt=aff&sl=34&s=AF-ID%2860069385%29+AND+SUBJAREA%28BUSI%29&cl=t&offset="+str(offset)+"&origin=resultslist&ss=plf-f&ws=r-f&ps=r-f&cs=r-f&cc=10&txGid=63057a16ac829d4c9815766603be94a4"
    ...
    for item in result.findAll('tr', {"class": "searchArea"}):
        ...
        link = h2['href']
        se = db.cursor()
        se.execute("SELECT Title FROM businessmanagement_and_accounting" )
        rs = se.fetchall()
        ...
        if judul not in out :
            ...
            if ze == 3:
                ...
                print("jumlah data business,management_and_accounting =" + str(len(data3)))
    ...
    offset = i
     ...
     sql = "INSERT INTO businessmanagement_and_accounting(Title, Author, Detail, Date_Released, Source, Link_Source, Cited_By) VALUES (%s, %s, %s, %s, %s, %s, %s)" 
     print(len(datae))   
            # try:
     i = 0
     for values in datae:
