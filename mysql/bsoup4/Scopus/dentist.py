...
while page is not None:
    url = "https://www.scopus.com/results/results.uri?sort=plf-f&src=s&sid=c5aacb3f15b99fa65da04c4814ecc33a&sot=aff&sdt=aff&sl=34&s=AF-ID%2860069385%29+AND+SUBJAREA%28DENT%29&cl=t&offset="+str(offset)+"&origin=resultslist&ss=plf-f&ws=r-f&ps=r-f&cs=r-f&cc=10&txGid=5ada12e0e240908a59d453bc8e7b2cf2"
    ...
    for item in result.findAll('tr', {"class": "searchArea"}):
        ...
        judul = h2.text
        se = db.cursor()
        se.execute("SELECT Title FROM dentistry" )
        rs = se.fetchall()
        ...
        if judul not in out :
            ...
            if ze == 3 :
                ...
                print("jumlah data dentistry =" + str(len(data3)))
    ...
    offset = i
     ...
     sql = "INSERT INTO dentistry(Title, Author, Detail, Date_Released, Source, Link_Source, Cited_By) VALUES (%s, %s, %s, %s, %s, %s, %s)" 
     print(len(datae))   
            # try:
     i = 0
     for values in datae:
