...
while page is not None:
    url = "https://www.scopus.com/results/results.uri?sort=plf-f&src=s&sid=271b10eca07c4bd0d61da50a993129b5&sot=aff&sdt=aff&sl=34&s=AF-ID%2860069385%29+AND+SUBJAREA%28ECON%29&cl=t&offset="+str(offset)+"&origin=resultslist&ss=plf-f&ws=r-f&ps=r-f&cs=r-f&cc=10&txGid=b0691021446335c9b1988ebd86e6866d"
    ...
    for item in result.findAll('tr', {"class": "searchArea"}):
        ...
        se = db.cursor()
        se.execute("SELECT Title FROM economics,econometrics_and_finance" )
        rs = se.fetchall()
        ...
        if judul not in out :
            ...
            print("jumlah data economics,econometrics_and_finance =" + str(len(data3)))
    ...
    offset = i
     ...
     sql = "INSERT INTO economics,econometrics_and_finance(Title, Author, Detail, Date_Released, Source, Link_Source, Cited_By) VALUES (%s, %s, %s, %s, %s, %s, %s)" 
     print(len(datae))   
            # try:
     i = 0
     for values in datae:
