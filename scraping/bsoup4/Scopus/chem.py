...
while page is not None:
    url = "https://www.scopus.com/results/results.uri?sort=plf-f&src=s&sid=6a246f2afebbcb3aed2c63d6b724fa3e&sot=aff&sdt=aff&sl=34&s=AF-ID%2860069385%29+AND+SUBJAREA%28CHEM%29&cl=t&offset="+str(offset)+"&origin=resultslist&ss=plf-f&ws=r-f&ps=r-f&cs=r-f&cc=10&txGid=01687968593fa119e5abd64ab6b54ffb"
    ...
    for item in result.findAll('tr', {"class": "searchArea"}):
        ...
        se = db.cursor()
        se.execute("SELECT Title FROM chemistry" )
        rs = se.fetchall()
        ...
        if judul not in out :
            ...
            print("jumlah data chemistry =" + str(len(data3)))
            sql = "INSERT INTO chemistry(Title, Author, Detail, Date_Released, Source, Link_Source, Cited_By) VALUES (%s, %s, %s, %s, %s, %s, %s)" 
