...
while page is not None:
    url = "https://www.scopus.com/results/results.uri?sort=plf-f&src=s&sid=7bfb818fe896cceb947e7235739eb61a&sot=aff&sdt=aff&sl=34&s=AF-ID%2860069385%29+AND+SUBJAREA%28EART%29&cl=t&offset="+str(offset)+"&origin=resultslist&ss=plf-f&ws=r-f&ps=r-f&cs=r-f&cc=10&txGid=09bbe45fa795a608528e8c1797ceb17b"
    ...
    for item in result.findAll('tr', {"class": "searchArea"}):
        ...
        judul = h2.text
        se = db.cursor()
        se.execute("SELECT Title FROM earth_and_planetarysciences" )
        rs = se.fetchall()
        ...
        if judul not in out :
            ...
            if ze == 3 :
                ...
                b.append(tgl)
                b.append(sou)
                b.append("https://www.scopus.com"+lins)
                b.append(cit)
    
                data2.append(tuple(b))
                print("jumlah data earth_and_planetarysciences =" + str(len(data2)))
                # try:
                if i == 1:
    ...
    offset = i
     ...
     sql = "INSERT INTO earth_and_planetarysciences(Title, Author, Detail, Date_Released, Source, Link_Source, Cited_By) VALUES (%s, %s, %s, %s, %s, %s, %s)" 
     print(len(datae))   
            # try:
     i = 0
     for values in datae:
