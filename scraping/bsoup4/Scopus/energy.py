...
while page is not None:
    url = "https://www.scopus.com/results/results.uri?sort=plf-f&src=s&sid=01b31e361aa7644dc59f7d92f0b27faa&sot=aff&sdt=aff&sl=34&s=AF-ID%2860069385%29+AND+SUBJAREA%28ENER%29&cl=t&offset="+str(offset)+"&origin=resultslist&ss=plf-f&ws=r-f&ps=r-f&cs=r-f&cc=10&txGid=1333d8a0d4654ea4627e2c9366c4d8ae"
    ...
    for item in result.findAll('tr', {"class": "searchArea"}):
        ...
        se = db.cursor()
        se.execute("SELECT Title FROM energy" )
        rs = se.fetchall()
        ...
        if judul not in out :
            ...
            print("jumlah data energy =" + str(len(data3)))
    ...
    offset = i
     ...
     sql = "INSERT INTO energy(Title, Author, Detail, Date_Released, Source, Link_Source, Cited_By) VALUES (%s, %s, %s, %s, %s, %s, %s)" 
     print(len(datae))   
            # try:
     i = 0
     for values in datae:
