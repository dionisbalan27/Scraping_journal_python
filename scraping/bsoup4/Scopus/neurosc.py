...
while page is not None:
    url = "https://www.scopus.com/results/results.uri?sort=plf-f&src=s&sid=7a9b824fcffbbb3dd8e41360999d4afa&sot=aff&sdt=aff&sl=34&s=AF-ID%2860069385%29+AND+SUBJAREA%28NEUR%29&cl=t&offset="+str(offset)+"&origin=resultslist&ss=plf-f&ws=r-f&ps=r-f&cs=r-f&cc=10&txGid=2c9450c45540a6f7f77b93f343dc5e98"
    ...
    for item in result.findAll('tr', {"class": "searchArea"}):
        ...
        link = h2['href']
        se = db.cursor()
        se.execute("SELECT Title FROM neuroscience" )
        rs = se.fetchall()
        ...
        if judul not in out :
            ...
            if ze == 3 :
                ...
                print("jumlah data neuroscience =" + str(len(data3)))
                sql = "INSERT INTO neuroscience(Title, Author, Detail, Date_Released, Source, Link_Source, Cited_By) VALUES (%s, %s, %s, %s, %s, %s, %s)" 
