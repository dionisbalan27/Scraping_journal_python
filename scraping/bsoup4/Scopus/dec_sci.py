...
while page is not None:
    url = "https://www.scopus.com/results/results.uri?sort=plf-f&src=s&sid=2ac0cd0368154818846167687b5429ba&sot=aff&sdt=aff&sl=34&s=AF-ID%2860069385%29+AND+SUBJAREA%28DECI%29&cl=t&offset="+str(offset)+"&origin=resultslist&ss=plf-f&ws=r-f&ps=r-f&cs=r-f&cc=10&txGid=e7bbb9e7bc8c4d80a12bc70b9f64f463"
    ...
    for item in result.findAll('tr', {"class": "searchArea"}):
        ...
        se = db.cursor()
        se.execute("SELECT Title FROM decision_sciences" )
        rs = se.fetchall()
        ...
        if judul not in out :
            ...
            print("jumlah data decision_sciences =" + str(len(data3)))
    ...
    offset = i
     ...
     sql = "INSERT INTO decision_sciences(Title, Author, Detail, Date_Released, Source, Link_Source, Cited_By) VALUES (%s, %s, %s, %s, %s, %s, %s)" 
     print(len(datae))   
            # try:
     i = 0
     for values in datae:
