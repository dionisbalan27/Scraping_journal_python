...
while page is not None:
    url = "https://www.scopus.com/results/results.uri?sort=plf-f&src=s&sid=c9a3b4284b72a499ce0c03e479b19993&sot=aff&sdt=aff&sl=34&s=AF-ID%2860069385%29+AND+SUBJAREA%28MEDI%29&cl=t&offset="+str(offset)+"&origin=resultslist&ss=plf-f&ws=r-f&ps=r-f&cs=r-f&cc=10&txGid=ab6fb003170d26b6671aaaea09d2cea3"
    ...
    page=bsObj.find("span", {"class": "ddmAuthorList"})
    ...
    for item in result.findAll('tr', {"class": "searchArea"}):
        ...
        judul = h2.text
        se = db.cursor()
        se.execute("SELECT Title FROM medicine" )
        rs = se.fetchall()
        ...
        if judul not in out :
            ...
            print("jumlah data medicine =" + str(len(data3)))
    ...
    offset = i
     ...
     sql = "INSERT INTO medicine(Title, Author, Detail, Date_Released, Source, Link_Source, Cited_By) VALUES (%s, %s, %s, %s, %s, %s, %s)" 
     print(len(datae))   
            # try:
     i = 0
     for values in datae:
