...
while page is not None:
    url = "https://www.scopus.com/results/results.uri?sort=plf-f&src=s&sid=cb9695f107c4fe69f514a7e534e3e53b&sot=aff&sdt=aff&sl=34&s=AF-ID%2860069385%29+AND+SUBJAREA%28MATE%29&cl=t&offset="+str(offset)+"&origin=resultslist&ss=plf-f&ws=r-f&ps=r-f&cs=r-f&cc=10&txGid=d5173a8ed2853dc68d69760866f05072"
    ...
    for item in result.findAll('tr', {"class": "searchArea"}):
        ...
        se = db.cursor()
        se.execute("SELECT Title FROM materials_science" )
        rs = se.fetchall()
        ...
        if judul not in out :
            ...
            print("jumlah data materials_science =" + str(len(data3)))
            if i == 1:
                    d = []
                    d.append(ppii)
    ...
    offset = i
     ...
     sql = "INSERT INTO materials_science(Title, Author, Detail, Date_Released, Source, Link_Source, Cited_By) VALUES (%s, %s, %s, %s, %s, %s, %s)" 
     print(len(datae))   
            # try:
     i = 0
     for values in datae:
