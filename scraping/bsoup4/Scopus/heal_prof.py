...
while page is not None:
    url = "https://www.scopus.com/results/results.uri?sort=plf-f&src=s&sid=83c98d6c8ce96435fcbc4a6cc35707f1&sot=aff&sdt=aff&sl=34&s=AF-ID%2860069385%29+AND+SUBJAREA%28HEAL%29&cl=t&offset="+str(offset)+"&origin=resultslist&ss=plf-f&ws=r-f&ps=r-f&cs=r-f&cc=10&txGid=71bc39568a73c37d9981ee99c224587e"
    ...
    for item in result.findAll('tr', {"class": "searchArea"}):
        ...
        se = db.cursor()
        se.execute("SELECT Title FROM health_professions" )
        rs = se.fetchall()
        ...
        if judul not in out :
            ...
            print("jumlah data health_professions =" + str(len(data3)))
    ...
    offset = i
     ...
     sql = "INSERT INTO health_professions(Title, Author, Detail, Date_Released, Source, Link_Source, Cited_By) VALUES (%s, %s, %s, %s, %s, %s, %s)" 
     print(len(datae))   
            # try:
     i = 0
     for values in datae:
