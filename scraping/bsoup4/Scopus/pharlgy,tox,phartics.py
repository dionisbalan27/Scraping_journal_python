...
while page is not None:
    url = "https://www.scopus.com/results/results.uri?sort=plf-f&src=s&sid=5b8d880c6f4bc0d3c38d74dff814ee58&sot=aff&sdt=aff&sl=34&s=AF-ID%2860069385%29+AND+SUBJAREA%28PHAR%29&cl=t&offset="+str(offset)+"&origin=resultslist&ss=plf-f&ws=r-f&ps=r-f&cs=r-f&cc=10&txGid=71e2ab74e6c89990b14de8ba21c00812"
    ...
    for item in result.findAll('tr', {"class": "searchArea"}):
        ...
        se = db.cursor()
        se.execute("SELECT Title FROM pharmacology,toxicology_and_pharmaceutics" )
        rs = se.fetchall()
    ...
    offset = i
     ...
     sql = "INSERT INTO pharmacology,toxicology_and_pharmaceutics(Title, Author, Detail, Date_Released, Source, Link_Source, Cited_By) VALUES (%s, %s, %s, %s, %s, %s, %s)" 
     print(len(datae))   
            # try:
     i = 0
     for values in datae:
