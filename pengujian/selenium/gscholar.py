...
if offset <= 999999 :
    ...
    # url = "https://www.sciencedirect.com/search?affiliations=diponegoro%20university"
        ...
        for res in driver.find_elements_by_xpath('//table[@class="uk-table"]/tbody/tr'):
            ...
            chrome_path = r"C:\Users\Asus\Documents\Python Scripts\selenium\chromedriver.exe"
            driver = webdriver.Chrome(chrome_path)
            driver.get(""+str(link_title[0:-8])+"documentsgs")
            ...
            for res in driver.find_elements_by_xpath('//table[@class="uk-table"]/tbody/tr'):
                ...
                if len(res.find_elements_by_xpath('.//dd'))!=2 :
                    h7=res.find_element_by_xpath('.//a').get_attribute('href');h8="none"
                else :
                        h7=res.find_elements_by_xpath('.//dd')[0].text.replace("</dd>","").replace("<dd>","").replace(", ...","")
                        h8=res.find_elements_by_xpath('.//dd')[1].text.replace('<dd class="indexed-by">\n','').replace("</dd>","").strip()
